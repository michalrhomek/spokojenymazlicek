...
{if $smarty.const._PS_VERSION_ >= 1.6 && isset($tabs) && $tabs|count}
				...
				{elseif $key == 'input'}
					...
					{foreach $field as $input}
						...
						{if $input.type == 'hidden'}
							<input type="hidden" name="{$input.name}" {if isset($input.id)}id="{$input.id}"{/if} value="{$fields_value[$input.name]|escape:'html':'UTF-8'}" />
						{else}
							{block name="label"}
							...
							{block name="field"}
								<div class="col-lg-{if isset($input.col)}{$input.col|intval}{else}9{/if}{if !isset($input.label) && $input.type != 'html'} col-lg-offset-3{/if}">
								{block name="input"}
													...
													{literal}
														<script type="text/javascript">
															$().ready(function () {
																...
																$('#'+input_id).tagify({delimiters: [13,44], addTagPrompt: '{/literal}{l s='Add tag' mod='pms_gopay_extra'}{literal}'});
								...
								{elseif $input.type == 'logo'}
									{if isset($input.lang) AND $input.lang}
										{foreach $languages as $language}
											...
											<div class="form-group translatable-field lang-{$language.id_lang}"{if $language.id_lang != $defaultFormLanguage} style="display:none;"{/if}>
												<div class="col-lg-9">
											{/if}
												<input type="file" id="logo_input_{$language.id_lang}" class="hide" onchange="uploadLogo({$language.id_lang});" name="logo_input_{$language.id_lang}"{if isset($input.accept)}  accept="{$input.accept}"{/if} data-id_lang="{$language.id_lang}" />
												<input type="hidden" id="payment_logo_{$language.id_lang}" name="payment_logo_{$language.id_lang}" value="{$fields_value[$input.name][$language.id_lang]}" />
												<div class="dummyfile input-group">
													<span class="input-group-addon"><i class="icon-file"></i></span>
													<input type="text" name="logo_name_{$language.id_lang}" id="logo_name_{$language.id_lang}" value="{$fields_value[$input.name][$language.id_lang]}" readonly />
													<span class="input-group-btn">
														<button type="button" name="submitAddAttachments" class="btn btn-default" data-id_lang="{$language.id_lang}">
															<i class="icon-folder-open"></i> {l s='Choose a file' mod='pms_gopay_extra'}
														</button>
													</span>
												</div>
												<p class="help-block">
														{l s='Format:' mod='pms_gopay_extra'} JPG, GIF, PNG..
												</p>
											{if $languages|count > 1}
												</div>
												<div class="col-lg-2">
									...
									{else}
										<div class="col-lg-9">
											<input type="file" id="logo_input_" class="hide" onchange="uploadLogo();" name="logo_input"{if isset($input.accept)}  accept="{$input.accept}"{/if} />
											<input type="hidden" id="payment_logo" name="payment_logo" value="{$fields_value[$input.name]}" />
											<div class="dummyfile input-group">
												<span class="input-group-addon"><i class="icon-file"></i></span>
												<input type="text" name="logo_name" id="logo_name" value="{$fields_value[$input.name]}" readonly />
												<span class="input-group-btn">
													<button type="button" name="submitAddAttachments" class="btn btn-default" data-id_lang="">
														<i class="icon-folder-open"></i> {l s='Choose a file' mod='pms_gopay_extra'}
													</button>
												</span>
											</div>
								...
								{elseif $input.type == 'maintenance_ip'}
									<div class="row">
										...
										<div class="col-lg-9">
											<div class="row">
												<div class="col-lg-8">
													<input type="text"{if isset($input.id)} id="{$input.id}"{/if} size="{if isset($input.size)}{$input.size|intval}{else}5{/if}" name="{$input.name}" value="{$fields_value[$input.name]}" />
												</div>
												<div class="col-lg-1">
													{$input.link_remove_ip}
												</div>
											</div>
										</div>
									</div>
										...
										<div class="col-lg-9">
											<div class="form-control-static row">
												<div class="col-xs-6">
													...
													<a href="#" id="addSwap" class="btn btn-default btn-block">{l s='Add' mod='pms_gopay_extra'} <i class="icon-arrow-right"></i></a>
												</div>
												<div class="col-xs-6">
													...
													<a href="#" id="removeSwap" class="btn btn-default btn-block"><i class="icon-arrow-left"></i> {l s='Remove' mod='pms_gopay_extra'}</a>
												</div>
											</div>
										</div>
									</div>
								{elseif $input.type == 'select'}
									{assign var=name value={$input.name|replace:'[]':''}}
									{if isset($input.options.query) && !$input.options.query && isset($input.empty_message)}
										{$input.empty_message}
										{$input.required = false}
										{$input.desc = null}
									{else}
										<select name="{$name|escape:'html':'utf-8'}{if isset($input.multiple)}[]{/if}"
												class="{if isset($input.class)}{$input.class|escape:'html':'utf-8'}{/if} fixed-width-xl"
												id="{if isset($input.id)}{$input.id|escape:'html':'utf-8'}{else}{$name|escape:'html':'utf-8'}{/if}"
											...
											{if isset($input.options.default)}
												<option style="direction: rtl;" value="{$input.options.default.value|escape:'html':'utf-8'}">{$input.options.default.label}</option>
											{/if}
														...
														{foreach $optiongroup[$input.options.options.query] as $option}
															<option value="{$option[$input.options.options.id]}"
																{if isset($input.multiple)}
																	{foreach $fields_value[$name] as $field_value}
																		{if $field_value == $option[$input.options.options.id]}selected="selected"{/if}
																	{/foreach}
																{else}
																	{if $fields_value[$name] == $option[$input.options.options.id]}selected="selected"{/if}
																{/if}
											...
											{else}
												{foreach $input.options.query AS $option}
													{if is_object($option)}
														<option value="{$option->$input.options.id}"
															{if isset($input.multiple)}
																...
																{/foreach}
															{else}
																{if $fields_value[$name] == $option->$input.options.id}
																	selected="selected"
																{/if}
															{/if}
													...
													{else}
														<option value="{$option[$input.options.id]}"
{*  moje uprava *}
															{if isset($option.data) && isset($option.data['value']) && $option.data['value']}
																data-{$option.data['name']}="{$option.data['value']}"
															{/if}
{*  moje uprava *}
															{if isset($input.multiple)}
																{foreach $fields_value[$name] as $field_value}
...
*}
		{foreach $input.values as $value}
			<div class="radio {if isset($input.class)}{$input.class}{/if}">
				{strip}
				<label>
					<input type="radio"	name="{$input.name}" id="{$value.id}" value="{$value.value|escape:'html':'UTF-8'}"{if $fields_value[$input.name] == $value.value} checked="checked"{/if}{if isset($input.disabled) && $input.disabled} disabled="disabled"{/if}{if isset($value.disabled) && $value.disabled} disabled="disabled"{/if}/>
					{$value.label}
				</label>
				{/strip}
			</div>
								...
								{elseif $input.type == 'switch'}
									<span class="switch prestashop-switch fixed-width-lg">
										...
										<label {if $value.value == 1} for="{$input.name}_on"{else} for="{$input.name}_off"{/if}>
											{if $value.value == 1}
												{l s='Yes' mod='pms_gopay_extra'}
											{else}
												{l s='No' mod='pms_gopay_extra'}
											{/if}
										</label>
										{/strip}
										{/foreach}
								...
								{elseif $input.type == 'checkbox'}
									...
									{foreach $input.values.query as $value}
										...
										<div class="checkbox{if isset($input.expand) && strtolower($input.expand.default) == 'show'} hidden{/if}">
											{strip}
												<input type="checkbox" name="{$input.name}[]" id="{$id_checkbox}" class="{if isset($input.class)}{$input.class}{/if}"{if isset($value[$input.values.val])} value="{$value[$input.values.val]|escape:'html':'UTF-8'}"{/if}{if isset($value[$input.values.val]) && is_array($fields_value[$input.name]) && in_array($value[$input.values.val], $fields_value[$input.name])} checked="checked"{/if} />
									...
									<div class="row">
										<div class="col-lg-12">
											<button type="button" id="{$input.name}-btn-change" class="btn btn-default">
												<i class="icon-lock"></i>
												{l s='Change password' mod='pms_gopay_extra'}
											</button>
											<div id="{$input.name}-change-container" class="form-password-change well hide">
												<div class="form-group">
													<label for="old_passwd" class="control-label col-lg-2 required">
														{l s='Current password' mod='pms_gopay_extra'}
													</label>
													<div class="col-lg-10">
												...
												<div class="form-group">
													<label for="{$input.name}" class="required control-label col-lg-2">
														<span class="label-tooltip" data-toggle="tooltip" data-html="true" title="" data-original-title="{l s='Password should be at least 8 characters long.' mod='pms_gopay_extra'}">
															{l s='New password' mod='pms_gopay_extra'}
														</span>
													</label>
													<div class="col-lg-9">
														<div class="input-group fixed-width-lg">
												...
												<div class="form-group">
													<label for="{$input.name}2" class="required control-label col-lg-2">
														{l s='Confirm password' mod='pms_gopay_extra'}
													</label>
													<div class="col-lg-4">
														<div class="input-group fixed-width-lg">
															<span class="input-group-addon">
												...
												<div class="form-group">
													<div class="col-lg-10 col-lg-offset-2">
														...
														<button type="button" id="{$input.name}-generate-btn" class="btn btn-default">
															...
															{l s='Generate password' mod='pms_gopay_extra'}
														</button>
													</div>
												</div>
												<div class="form-group">
													<div class="col-lg-10 col-lg-offset-2">
														<p class="checkbox">
															<label for="{$input.name}-checkbox-mail">
																...
																{l s='Send me a new password to email' mod='pms_gopay_extra'}
															</label>
														</p>
													</div>
												</div>
												<div class="row">
													<div class="col-lg-12">
														<button type="button" id="{$input.name}-cancel-btn" class="btn btn-default">
															...
															{l s='Cancel' mod='pms_gopay_extra'}
														</button>
													</div>
												</div>
											</div>
										...
										<select name="{$key}" class="fixed-width-lg{if isset($input.class)} {$input.class}{/if}">
											...
											{if $key == 'months'}
												{*
													...
													{l s='January' mod='pms_gopay_extra'}
													{l s='February' mod='pms_gopay_extra'}
													{l s='March' mod='pms_gopay_extra'}
													{l s='April' mod='pms_gopay_extra'}
													{l s='May' mod='pms_gopay_extra'}
													{l s='June' mod='pms_gopay_extra'}
													{l s='July' mod='pms_gopay_extra'}
													{l s='August' mod='pms_gopay_extra'}
													{l s='September' mod='pms_gopay_extra'}
													{l s='October' mod='pms_gopay_extra'}
													{l s='November' mod='pms_gopay_extra'}
													{l s='December' mod='pms_gopay_extra'}
												*}
				...
				{if isset($fieldset['form']['submit']) || isset($fieldset['form']['buttons'])}
					<div class="panel-footer">
						...
						<a href="{$back_url|escape:'html':'UTF-8'}" class="btn btn-default" onclick="window.history.back();">
							<i class="process-icon-cancel"></i> {l s='Cancel' mod='pms_gopay_extra'}
						</a>
						{/if}
