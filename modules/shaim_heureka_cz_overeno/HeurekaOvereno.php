...
class HeurekaOvereno
{
    ...
    public function __construct($apiKey, $languageId = self::LANGUAGE_CZ, $path)
    {
        $this->return = $this->setApiKey($apiKey);
    ...
    public function send()
    {
        ...
        file_put_contents($this->path_log, '[' . date("Y-m-d H:i:s") . '] ' . $url . ' - ' . (($this->languageId == 1) ? 'cz' : 'sk') . ' - ' . $contents . PHP_EOL, FILE_APPEND);
        if ($contents == FALSE) {
