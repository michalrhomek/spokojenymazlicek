				...
				<div class="col-xs-12 col-md-3">
					...
					{else}
						<select id="id_contact" class="form-control" name="id_contact">
							...
							{foreach from=$contacts item=contact}
								{if $iter == 0}
								<option value="{$contact.id_contact|intval}"{if isset($smarty.request.id_contact) && $smarty.request.id_contact == $contact.id_contact} selected="selected"{/if}>{* $contact.name|escape:'html':'UTF-8' *} Zákaznická podpora</option>
								{$iter++}
								{/if}
							{/foreach}
						</select>
